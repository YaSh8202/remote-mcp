	...
	for (const [key, property] of Object.entries(properties)) {
		if (property && typeof property === "object") {
			...
			if (typeof prop.type === "string") {
				...
				param.type =
					prop.type.filter((t) => t !== "null").join(" | ") || "unknown";
			}
			...
			if (prop.enum && Array.isArray(prop.enum)) {
				param.type = "enum";
				param.description =
					`${param.description || ""} Options: ${prop.enum.join(", ")}`.trim();
			}
		...
		if (schema && typeof schema === "object") {
			...
			for (const [key, value] of entries) {
				if (value && typeof value === "object") {
					...
					if ("_def" in value) {
						const def = (value as { _def?: unknown })._def as Record<
							string,
							unknown
						>;
						...
						if (def.typeName && typeof def.typeName === "string") {
							switch (def.typeName) {
								...
								case "ZodOptional": {
									...
									const optionalInnerType = def.innerType as
										| Record<string, unknown>
										| undefined;
									if (
										optionalInnerType?._def &&
										typeof (optionalInnerType._def as Record<string, unknown>)
											.typeName === "string"
									) {
										param.type = (
											(optionalInnerType._def as Record<string, unknown>)
												.typeName as string
										)
											.replace("Zod", "")
											.toLowerCase();
								...
								case "ZodDefault": {
									...
									const defaultInnerType = def.innerType as
										| Record<string, unknown>
										| undefined;
									if (
										defaultInnerType?._def &&
										typeof (defaultInnerType._def as Record<string, unknown>)
											.typeName === "string"
									) {
										param.type = (
											(defaultInnerType._def as Record<string, unknown>)
												.typeName as string
										)
											.replace("Zod", "")
											.toLowerCase();
