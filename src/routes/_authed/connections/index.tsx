...
import { NewConnectionDialog } from '@/routes/_authed/servers/-components/new-connection-dialog'
...
import { type ConnectionWithUsage, ConnectionsTable } from './-components/connections-table'
...
function RouteComponent() {
  ...
  const [selectedConnection, setSelectedConnection] = useState<ConnectionWithUsage | null>(null)

  const trpc = useTRPC()
  
  // Fetch connections with usage count
  const { data: connections, isLoading, refetch } = useQuery(
    trpc.appConnection.getAllConnectionsWithUsage.queryOptions()
  )
    ...
    <div className="container mx-auto p-6 space-y-8">
      ...
      <div className="space-y-2">
        <h1 className="text-2xl font-semibold tracking-tight flex items-center gap-3">
          ...
          Connections {connectionsData.length > 0 && `(${connectionsData.length})`}
        </h1>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-muted-foreground mb-4 text-center max-w-md">
              Create your first connection to start integrating applications with your MCP servers.
            </p>
        ...
        message={
          selectedConnection ? (
            <>
              ...
              <strong>{selectedConnection.displayName}</strong>? This action cannot be undone.
              {selectedConnection.usageCount > 0 && (
                <div className="mt-2 p-2 bg-destructive/10 rounded border border-destructive/20">
                  <strong>Warning:</strong> This connection is currently used by{' '}
                  {selectedConnection.usageCount} MCP server(s). Deleting it may break those integrations.
                </div>
              )}
            </>
          ) : (
