...
export const channelsListTool = createParameterizedTool({
  ...
  paramsSchema: {
    channel_types: z.string().describe("Comma-separated channel types. Allowed values: 'mpim', 'im', 'public_channel', 'private_channel'. Example: 'public_channel,private_channel,im'"),
    sort: z.string().optional().describe("Type of sorting. Allowed values: 'popularity' - sort by number of members/participants in each channel."),
    limit: z.number().default(100).describe("The maximum number of items to return. Must be an integer between 1 and 1000 (maximum 999)."),
    cursor: z.string().optional().describe("Cursor for pagination. Use the value of the last row and column in the response as next_cursor field returned from the previous request."),
  },
  callback: async (params, extra) => {
    try {
      ...
      type Channel = NonNullable<Awaited<ReturnType<typeof client.conversations.list>>['channels']>[number];
      let channels: Channel[] = [];
      let nextCursor = "";
      
      // Handle public and private channels
      if (channelTypes.includes('public_channel') || channelTypes.includes('private_channel')) {
